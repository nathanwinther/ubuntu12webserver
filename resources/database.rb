...
action :create do
  ...
  directory node['ubuntu12webserver']['chef_run_once_directory'] do
    action :create
    owner 'root'
    group 'root'
    mode '0755'
  ...
  file node['ubuntu12webserver']['chef_run_once_directory'] + '/remote_root' do
  ...
  file node['ubuntu12webserver']['chef_run_once_directory'] + '/root_password' do
    action :create_if_missing
